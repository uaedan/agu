...
AIM: 
      = h) High Signal portion of psERM
        ...
        . stp interval is 10 smp for search (see: 
...
#   tmpTRM=tmp[n2:n90]
    tmpTRM=tmp[n10:n90]
#   lll=float(len(tmp))
#   ll2=float(len(tmpTRM))
    MAD=float(tmpTRM.mad())
...
#   print "%.4f"%minS,"%.4f"%minE,bS,bE,"%f"%kurt,"%f"%MAD,"%f"%lll,"%f"%ll2
    print "%.4f"%minS,"%.4f"%minE,bS,bE,"%f"%kurt,"%.10lf"%MAD
#   bS=bS+100 # Dubious
#   bE=bE+100 # Dubious
#   bS=bS+1
#   bE=bE+1
